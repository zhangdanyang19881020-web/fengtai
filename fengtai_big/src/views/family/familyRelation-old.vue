...
const renderContent: RenderContentFunction = (h, { node, data }) => {
  return h('div', { class: 'custom-tree-node' }, [
    data.editing
      ...
      : h('span', { onDblclick: () => (data.editing = true) }, data.label),
    h('div', null, [
      !data.editing &&
        h(
          ElButton,
          { type: 'warning', link: true, size: 'small', style: 'margin-left:4px', onClick: () => (data.editing = true) },
          () => 'Edit'
        ),
      data.editing &&
        h(
          ElButton,
          { type: 'success', link: true, size: 'small', style: 'margin-left:4px', onClick: () => (data.editing = false) },
          () => 'Save'
        )
    ])
  ])
