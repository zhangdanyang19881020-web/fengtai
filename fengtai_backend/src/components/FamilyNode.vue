<template>
	<g>
		...
		<!-- 配偶矩形 -->
		<rect v-if="node.spouse" :x="x + nodeWidth/2 + 5" :y="y - nodeHeight/2" :width="nodeWidth" :height="nodeHeight"
			fill="#f7f0db" stroke="#8b4513" stroke-width="2" rx="5" style="cursor: pointer;" />

		<!-- 连接配偶的线 -->
		<line v-if="node.spouse" :x1="x + nodeWidth/2" :y1="y" :x2="x + nodeWidth/2 + 5" :y2="y" stroke="#8b4513"
			stroke-width="2" />
		...
		<!-- 配偶姓名 -->
		<text v-if="node.spouse" :x="x + nodeWidth/2 + 5 + nodeWidth/2" :y="y - 10" text-anchor="middle" fill="#8b4513"
			style="font-size: 14px; pointer-events: none; font-weight: bold;">
			{{ node.spouse }}
		</text>
